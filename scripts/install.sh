...
display_help() {
    echo -e "${GREEN} Usage:${NC} $0 <db_host> <db_port> <db_user> [db_password]"
    ...
    echo -e "  ${GREEN} [db_password]${NC}        Database password (optional)"
}
...
# If password is provided, set it as well
if [ $# -eq 4 ]; then
    export _DATABASE_PASSWORD_="$4"
fi
